...
8. **OPENAI BILLING (founder task, blocks everything)** — discovered 2026-07-06/08: the OpenAI account has zero quota (`insufficient_quota`), so every AI feature (diagnosis, voice, pricing, parts, bid-writer) is dead in production. The Vercel key was also stale/drifted (fixed — re-synced from .env.local), and every AI-calling UI used to lie about failures (fixed — all surfaces now show the real error). Founder must add billing at platform.openai.com/settings/billing.
